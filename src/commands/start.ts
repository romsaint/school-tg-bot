...
export async function startCommand(msg: TelegramBot.Message) {
    try {
        ...
        const text = `\`БОТ Который поможет вам с расписанием уроков. Он будет напоминать, когда какой урок и когда перемена. \`\n\`\`\`${day ? day : 'Воскресенье '}СЕГОДНЯ ОТДЫХ🥳\`\`\``
        ...
        if (lessons.length > 0) {
            ...
            for (let i = 0; i < lessons.length; i++) {
                ...
                if (dateNow >= dateStart && dateNow <= dateEnd) {
                    ...
                    if(diffInMinutes > 20) {
                        remainingTime = `⏳Потерпеть... осталось ${diffInMinutes} минут до конца ${currentLesson}`;
                    }else{
                        remainingTime = `⏳Кайфуем осталось ${diffInMinutes} минут до конца ${currentLesson}`;
                    }
                ...
                if (i === 1 || i === 2) {
                    scheduleText += `\`\`\`${i + 1}. ${lesson}${!lessons[i].homework ? ' - Не задано' :` - ${lessons[i].homework}` }\`\`\`\n🕒 С ${timeStart} - ${timeEnd}, \`перемена ${20} минут\`\n`;
            ...
            if (remainingTime) {
                ...
                if (nextLesson) {
                    ...
                    scheduleText += `\`\`\`Празднуем ЭТО ПОСЛЕДНИЙ УРОК 🥳\`\`\`\n`;
                }
            }
